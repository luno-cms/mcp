...
| Category | Tools |
|----------|--------|
| **A. Read-only** | `get_tenant_schema`, `get_project_overview`, `list_form_sets`, `get_form_set_schema`, `get_public_api_info`, `list_entries`, `get_entry`, `list_media`, `list_revisions`, `get_pub_preview_url`, `get_agent_run`, `get_change_plan`, `list_builtin_form_templates`, `validate_master_blueprint`, `list_master_entities`, `get_master_entity`, `list_master_records`, `search_admin_help`, `get_admin_help_article`, `get_login_branding`, `get_login_appearance`, `list_console_login_ip_allowlists`, `get_project_content_locales`, `get_funnel_status` |
...
| **D. Schema / blueprint** | `get_tenant_schema`, `get_form_set_schema`, `apply_form_blueprint`, `apply_builtin_form_template`, `validate_master_blueprint`, `apply_master_blueprint` |
