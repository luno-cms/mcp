...
|------|------------|----------------|
| T0 Registration | All 51 | Tool is registered; description non-empty; four annotation hints present |
| T1 Schema | All 51 | No-arg tools have empty `properties`. Others expose typed JSON Schema properties |
...
| T4 Safety | `safety: true` | `archive_form_set` dryRun/confirmToken; `publish_revision` `/publish`; `propose_change` does not apply; IP delete uses DELETE; `migrate_field_to_master_reference` / `rename_master_record_slug` reject `dryRun: false` (no API call) and post `dryRun: true` only. `create_contact_form` dryRun is asserted in `mcp-contract-quality.test.ts` (description + schema) |
| T5 Contract | All 51 | Every input property has `description`; every tool has `outputSchema`; representative Golden Path tools return `structuredContent` that parses |
