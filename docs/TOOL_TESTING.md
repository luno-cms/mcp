...
|------|------------|----------------|
| T0 Registration | All 52 | Tool is registered; description non-empty; four annotation hints present |
| T1 Schema | All 52 | No-arg tools have empty `properties`. Others expose typed JSON Schema properties |
...
| T5 Contract | All 52 | Every input property has `description`; every tool has `outputSchema`; representative Golden Path tools return `structuredContent` that parses |
